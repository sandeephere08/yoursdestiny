...
async def restart_bot(_, message):
    ...
    ac_chat = await get_active_chats()
    for x in ac_chat:
        try:
            await app.change_chat_title(x, f"⏳ {await get_lang(x)}")
        except:
            pass
    try:
        await app.stop()
        await app.start()
        await response.edit_text("✅ ʙᴏᴛ ʜᴀs ʙᴇᴇɴ ʀᴇsᴛᴀʀᴛᴇᴅ!")
        try:
            await app.send_message(
                config.LOGGER_ID,
                "✅ ʙᴏᴛ ʜᴀs ʙᴇᴇɴ ʀᴇsᴛᴀʀᴛᴇᴅ!",
            )
        except:
            pass
    except Exception as e:
        await response.edit_text(f"❌ ᴀɴ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀʀᴇᴅ: {e}")
        try:
            await app.send_message(
                config.LOGGER_ID,
